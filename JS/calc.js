...
const calcTutBtn = document.getElementById("calcTut");
...
// displaying sweetalert for tutorial on how to use calc
calcTut.addEventListener("click", () => {
	Swal.fire({
		...
		background: "#424652",
		color: "#fff",
		imageUrl: "../media/tut/calc_tut_gif_compressed_cropped.gif",
		imageWidth: "30rem",

		// Custom classes for styling
		customClass: {
			title: "tut-popup-title",
...
shape_name[0].onclick = () => {
	...
	inp.oninput = () => {
		let values = [
			"a" + '<sup style="font-size: 17px;">2</sup>',
			inp.value + '<sup style="font-size: 17px;">2</sup>',
			inp.value * inp.value,
		];
		let flag_num = 1;
...
shape_name[2].onclick = () => {
	...
	[inp, inp2].forEach(function (element) {
		element.addEventListener("input", function () {
			let values = [
				formula.innerHTML,
				`½  x  ${inp.value}  x  ${inp2.value}`,
				0.5 * inp.value * inp2.value,
			];
			let flag_num = 2;
			settingValues(flag_num, values);
		});
...
shape_name[3].onclick = () => {
	...
	[inp, inp2].forEach(function (element) {
		element.addEventListener("input", function () {
			let values = [
				formula.innerHTML,
				`π x ${inp.value}<sup style="font-size: 17px;">2</sup>`,
				((22 / 7) * (inp.value * inp.value)).toFixed(2),
			];
			let flag_num = 1;
			settingValues(flag_num, values);
		});
...
shape_name[5].onclick = () => {
	...
	[inp, inp2, inp3].forEach(function (element) {
		element.addEventListener("input", function () {
			let values = [
				formula.innerHTML,
				` ( ${inp.value} + ${inp2.value} )/ 2 * ${inp3.value}`,
				((inp.value * inp2.value) / 2) * inp3.value,
			];
			let flag_num = 3;
			settingValues(flag_num, values);
...
shape_name[6].onclick = () => {
	...
	[inp, inp2].forEach(function (element) {
		element.addEventListener("input", function () {
			let values = [
				formula.innerHTML,
				`π (${inp.value}<sup style="font-size: 13px;">2</sup> - ${inp2.value}<sup style="font-size: 13px;">2</sup>)`,
				(
					(22 / 7) *
...
shape_name[7].onclick = () => {
	...
	[inp, inp2].forEach(function (element) {
		element.addEventListener("input", function () {
			let values = [
				formula.innerHTML,
				`π x ${inp.value} x ${inp2.value}`,
				((22 / 7) * inp.value * inp2.value).toFixed(2),
			];
			let flag_num = 2;
			settingValues(flag_num, values);
		});
